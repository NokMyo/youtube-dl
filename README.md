...
지원 환경은 Windows 10/11 x64입니다. 시스템 DPI와 실제 작업 영역에 맞춰 창·컨트롤·아이콘 크기를 조정하며 키보드 `Tab` 이동을 지원합니다. 논리 프로세서 수에 따라 정보 조회와 다운로드 동시 작업 수를 자동 조절하고 백그라운드 작업 우선순위를 낮춰 저사양 PC에서도 화면 반응성을 유지합니다.
...
- 논리 프로세서 4개 이하에서는 조회·변환을 각각 1개만 실행하고, 여유가 있는 PC에서만 최대 메타데이터 4개 병렬 조회와 2개 병렬 다운로드
...
- 앱 안에서 최신 버전을 확인하고 SHA-256 검증 후 자동 교체·재시작하는 업데이트
...
첫 실행 시 별도 시작 화면이 표시되며 내장된 `yt-dlp`, `Deno`, `ffmpeg`, `ffprobe`가 `%LOCALAPPDATA%\Febius\Downrush\tools`에 자동으로 준비됩니다. 이후 실행에서는 도구 내용의 SHA-256 식별값만 빠르게 확인하고 준비된 도구를 재사용합니다. 앱 코드만 업데이트되고 도구 내용이 같으면 큰 압축 파일을 다시 풀지 않습니다.

업데이트 확인 창에서 설치를 승인하면 새 실행 파일과 릴리스 체크섬을 함께 내려받아 SHA-256이 일치하는지 검사합니다. 검증된 경우에만 현재 프로그램을 종료하고 실행 파일을 교체한 뒤 자동으로 다시 시작합니다. 다운로드나 검증에 실패하면 기존 실행 파일을 그대로 유지합니다.
